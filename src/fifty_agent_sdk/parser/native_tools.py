...
:class:`fifty_agent_sdk.llm.types.ChatMessage` (deferred to a later brief — the
...
The Protocol is kept distinct from :class:`fifty_agent_sdk.parser.base.Parser`
...
from fifty_agent_sdk.parser.base import ParseResult
...
class NativeToolsParser(Protocol):
    ...
    Distinct from :class:`fifty_agent_sdk.parser.base.Parser` because the input is
    a structured :class:`fifty_agent_sdk.llm.types.ChatResponse`, not a completion
    ...
    brief alongside the :class:`fifty_agent_sdk.llm.types.ChatMessage` schema
    extension.
    """
...
class NativeToolsParserStub:
    ...
    def parse(self, response: ChatResponse) -> ParseResult:
        ...
        raise NotImplementedError(
            ...
            "fifty_agent_sdk.parser.native_tools module docstring for the planned "
            "integration point."
        )
