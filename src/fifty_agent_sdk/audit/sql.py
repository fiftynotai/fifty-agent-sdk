...
Extras requirement
    ...
    Importing :mod:`fifty_agent_sdk` itself does NOT pull SQLAlchemy. The audit
    SQL surface is re-exported lazily from :mod:`fifty_agent_sdk.audit` and the
...
Schema lifecycle
    ...
    their own Alembic environment::
        ...
        # alembic/env.py
        from fifty_agent_sdk import audit_metadata
        target_metadata = audit_metadata

    Then::

        alembic revision --autogenerate -m "fifty_agent_sdk audit schema"

    The audit table uses a :class:`sqlalchemy.MetaData` object DISTINCT
    from :data:`fifty_agent_sdk.sql_metadata` (the conversation-state schema), so
...
Engine ownership
    ...
    :class:`fifty_agent_sdk.state.sql.SqlStateStore` are independent
...
Error wrapping contract
    ...
    SQLAlchemy base class) into :class:`fifty_agent_sdk.errors.StateStoreError`
    with:
...
try:
    ...
    raise ImportError(
        "fifty_agent_sdk.audit.sql requires SQLAlchemy. Install with: pip install 'fifty-agent-sdk[sql]'"
    ) from exc
...
Bound to the fixed name ``fifty_agent_sdk.audit`` (shared with
:mod:`fifty_agent_sdk.audit.console`). Successful writes log at ``DEBUG``;
...
class Base(DeclarativeBase):
    ...
    :class:`fifty_agent_sdk.state.sql.Base`: the audit table must be registerable
...
class AgentAuditLog(Base):
    ...
    treatment as :attr:`fifty_agent_sdk.state.sql.AgentSession.meta`."""
...
It is DISTINCT from :data:`fifty_agent_sdk.sql_metadata` (the conversation-state
...
class SqlAuditSink:
    ...
    Concurrency model:
        ...
        :class:`fifty_agent_sdk.state.sql.SqlStateStore` is intentionally OMITTED.
    ...
    Failure mode:
        ...
        :class:`fifty_agent_sdk.errors.StateStoreError` with ``context["wrapped"]``
