...
optional ``sql`` extra, so ``import fifty_agent_sdk.audit`` itself does NOT pull
SQLAlchemy.

Accessing :data:`SqlAuditSink` or :data:`audit_metadata` without
``fifty-agent-sdk[sql]`` installed raises a clear :class:`ImportError` referencing
the extras line.
"""
...
from fifty_agent_sdk.audit.console import ConsoleAuditSink
from fifty_agent_sdk.audit.protocol import AuditEvent, AuditSink
...
def __getattr__(name: str) -> Any:
    ...
    if name in {"SqlAuditSink", "audit_metadata"}:
        from fifty_agent_sdk.audit import sql

        return getattr(sql, name)
    raise AttributeError(f"module 'fifty_agent_sdk.audit' has no attribute {name!r}")
