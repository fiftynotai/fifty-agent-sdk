...
observability surface, and it is deliberately vendor-neutral: ``agent-sdk``
...
Two-tier wiring
    ...
    * Five **Runner-tier** hooks (``on_run_start``, ``on_run_end``,
      ...
      :class:`agent_sdk.runner.AgentRunner`.
    * Two **Loop-tier** hooks (``on_iteration``, ``on_llm_call``) fire from
      :class:`agent_sdk.loop.AgentLoop` — the ReACT iteration counter and
      the per-call :class:`~agent_sdk.llm.types.ChatRequest` /
      :class:`~agent_sdk.llm.types.ChatResponse` are loop-private and are
      NOT reconstructable from the public event stream.

    Construct ONE :class:`Hooks` instance and pass the SAME instance into
    BOTH :class:`agent_sdk.loop.AgentLoop` and
    :class:`agent_sdk.runner.AgentRunner`. The Runner does NOT forward its
...
Failure isolation
    ...
    under the fixed ``agent_sdk.observability`` logger, and swallows it.
...
Illustrative consumer wiring (NOT shipped — ``agent-sdk`` has no
...
_log: Final = structlog.get_logger("agent_sdk.observability")
"""Module-level structured logger.

Bound to the fixed name ``agent_sdk.observability`` (NOT ``__name__``) so
every hook-failure warning shares one logger name a consumer can route or
filter on — mirroring :mod:`agent_sdk.audit`'s fixed ``agent_sdk.audit``
logger.
"""
...
class Hooks:
    ...
    It is collaborator wiring, the same role :class:`agent_sdk.safety.
    ...
    Attributes:
        ...
        on_run_end: ``(session_id, duration_ms, error) -> Any``. Fires once
            ...
            :class:`~agent_sdk.errors.StateStoreError`, or a surfaced
            ...
            :class:`~agent_sdk.streaming.ErrorEvent` and drives ``on_error``
        ...
        on_llm_call: ``(session_id, request, response, duration_ms) -> Any``.
            ...
            response. In stream mode ``response`` is a :class:`~agent_sdk.
        ...
        on_tool_start: ``(session_id, tool_name, args) -> Any``. Fires once
            ...
            :class:`~agent_sdk.streaming.ToolStartedEvent`). Runner tier.
        ...
        on_error: ``(session_id, error, context) -> Any``. Fires on a
            ...
            :class:`~agent_sdk.streaming.ErrorEvent`) and on a state-store
            durability failure (the caught
            :class:`~agent_sdk.errors.StateStoreError`). ``context`` is a
...
async def invoke_hook(
    ...
    *args: Any,
    ...
    :class:`agent_sdk.runner.AgentRunner` and
    :class:`agent_sdk.loop.AgentLoop`, so the sync/async detection and the
    ...
    * A raising hook is caught: :class:`asyncio.CancelledError` is re-raised
      ...
      ``hook.invoke_failed``) under the ``agent_sdk.observability`` logger
