...
:data:`agent_sdk.prompts.JSON_MODE_OUTPUT_FORMAT`:
...
:class:`agent_sdk.errors.ParserError` with ``error_phase="schema_validation"``.
...
:mod:`agent_sdk.parser.prose_mode` so the prose parser can recover JSON in
...
class JsonModeParser:
    ...
    (via :func:`agent_sdk.prompts.json_mode_template`). Consumes the schema
    defined in :data:`agent_sdk.prompts.JSON_MODE_OUTPUT_FORMAT`.
