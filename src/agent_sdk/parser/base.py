...
Implementations MUST raise :class:`agent_sdk.errors.ParserError` on malformed
...
class ThoughtAction(BaseModel):
    ...
    Attributes:
        ...
        tool_call: The tool invocation request, in the SDK's
            provider-agnostic :class:`agent_sdk.llm.types.ToolCall` shape.
    """
...
class Parser(Protocol):
    ...
    Implementations MUST raise :class:`agent_sdk.errors.ParserError` on
    ...
    def parse(self, completion: str) -> ParseResult:
        ...
        Raises:
            agent_sdk.errors.ParserError: When the completion cannot be
