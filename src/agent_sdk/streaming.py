...
Reserved future events:
    ...
    :class:`agent_sdk.tools.protocol.Tool` has no progress channel. A future
...
class ActionEvent(_EventBase):
    ...
    returned a :class:`agent_sdk.parser.base.ThoughtAction`.
...
class ToolStartedEvent(_EventBase):
    ...
    Emitted right before :meth:`agent_sdk.tools.registry.Registry.invoke`
...
class ToolProgressEvent(_EventBase):
    ...
    widens the :class:`agent_sdk.tools.protocol.Tool` contract to support
...
class ToolFailedEvent(_EventBase):
    ...
    * :class:`agent_sdk.errors.ToolNotFound` raised (hallucinated tool name).
    * :class:`agent_sdk.errors.ToolTimeout` raised (per-tool timeout).
...
class TokenEvent(_EventBase):
    ...
    Only emitted when :class:`agent_sdk.loop.AgentLoop` was constructed with
    ``stream=True`` AND the iteration's parse resolved to a
    :class:`agent_sdk.parser.base.FinalAnswer`. Intermediate
...
class ErrorEvent(_EventBase):
    ...
    :attr:`agent_sdk.safety.SafetyConfig.fallback_message`. Consumers can
    pattern-match on ``error_type`` to take application-specific action
    (for instance, re-raising :class:`agent_sdk.errors.MaxIterationsExceeded`
