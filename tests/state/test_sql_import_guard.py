...
top-level ``sqlalchemy`` import inside :mod:`agent_sdk.state.sql` fails,
...
def sqlalchemy_unavailable(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    Removes any already-imported ``sqlalchemy*`` and ``agent_sdk.state.sql``
    ...
    reloading the real :mod:`agent_sdk.state.sql` so the rest of the
    test session sees a working SQL surface.
    """
    # Strip every cached sqlalchemy* module + agent_sdk.state.sql so the
    # next import path goes through our blocked entry. We collect first,
    # then mutate, to avoid mutating during iteration.
    removed: list[str] = [
        name
        for name in list(sys.modules)
        if name == "sqlalchemy" or name.startswith("sqlalchemy.") or name == "agent_sdk.state.sql"
    ]
    for name in removed:
    ...
    # agent_sdk.state.sql so the next caller re-imports it cleanly.
    sys.modules.pop("agent_sdk.state.sql", None)


def test_import_without_sqlalchemy_raises_clear_error(
    sqlalchemy_unavailable: None,
) -> None:
    """Direct ``import agent_sdk.state.sql`` without SQLAlchemy gives a clear ImportError."""
    with pytest.raises(ImportError) as exc_info:
        importlib.import_module("agent_sdk.state.sql")
...
def test_lazy_attribute_access_on_state_package_triggers_extras_hint(
    ...
    """``agent_sdk.state.SqlStateStore`` (lazy) raises the same ImportError."""
    # `agent_sdk.state` itself does not import sqlalchemy, so a fresh import
...
def test_top_level_lazy_attribute_access_triggers_extras_hint(
    ...
    """``agent_sdk.SqlStateStore`` (top-level lazy) raises the same ImportError."""
...
def test_lazy_attribute_access_for_sql_metadata_triggers_extras_hint(
    ...
    """``agent_sdk.sql_metadata`` lazy access also surfaces the extras hint."""
    sys.modules.pop("agent_sdk", None)
    pkg = importlib.import_module("agent_sdk")
...
def test_top_level_unknown_attribute_still_raises_attribute_error() -> None:
    ...
    with pytest.raises(AttributeError):
        _ = agent_sdk.DefinitelyNotASymbol
