...
and the ``fifty-agent-sdk[redis]`` extras line. The in-memory store and the
rest of the SDK remain importable.

These tests simulate "missing extra" by patching ``sys.modules`` so the
top-level ``redis`` import inside :mod:`fifty_agent_sdk.state.redis` fails,
...
def redis_unavailable(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    Removes any already-imported ``redis*`` and ``fifty_agent_sdk.state.redis``
    ...
    :mod:`fifty_agent_sdk.state.redis` so the rest of the test session sees a
    working Redis surface.
    """
    # Strip every cached redis* module + fifty_agent_sdk.state.redis so the next
    # import path goes through our blocked entry. We collect first, then
    # mutate, to avoid mutating during iteration.
    removed: list[str] = [
        name
        for name in list(sys.modules)
        if name == "redis" or name.startswith("redis.") or name == "fifty_agent_sdk.state.redis"
    ]
    for name in removed:
    ...
    # fifty_agent_sdk.state.redis so the next caller re-imports it cleanly.
    sys.modules.pop("fifty_agent_sdk.state.redis", None)


def test_import_without_redis_raises_clear_error(
    redis_unavailable: None,
) -> None:
    """Direct ``import fifty_agent_sdk.state.redis`` without redis gives a clear ImportError."""
    with pytest.raises(ImportError) as exc_info:
        importlib.import_module("fifty_agent_sdk.state.redis")
    msg = str(exc_info.value)
    assert "redis" in msg.lower()
    assert "fifty-agent-sdk[redis]" in msg


def test_lazy_attribute_access_on_state_package_triggers_extras_hint(
    redis_unavailable: None,
) -> None:
    """``fifty_agent_sdk.state.RedisStateStore`` (lazy) raises the same ImportError."""
    # `fifty_agent_sdk.state` itself does not import redis, so a fresh import
...
def test_top_level_lazy_attribute_access_triggers_extras_hint(
    ...
    """``fifty_agent_sdk.RedisStateStore`` (top-level lazy) raises the same ImportError."""
