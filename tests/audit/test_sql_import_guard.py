...
top-level ``sqlalchemy`` import inside :mod:`fifty_agent_sdk.audit.sql` fails,
...
def sqlalchemy_unavailable(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    Removes any already-imported ``sqlalchemy*`` and ``fifty_agent_sdk.audit.sql``
    ...
    removed: list[str] = [
        ...
        or name == "fifty_agent_sdk.audit.sql"
    ]
    for name in removed:
    ...
    # back. We also pop our half-built fifty_agent_sdk.audit.sql so the next
    # caller re-imports it cleanly.
    sys.modules.pop("fifty_agent_sdk.audit.sql", None)


def test_import_without_sqlalchemy_raises_clear_error(
    sqlalchemy_unavailable: None,
) -> None:
    """Direct ``import fifty_agent_sdk.audit.sql`` without SQLAlchemy gives a clear error."""
    with pytest.raises(ImportError) as exc_info:
        importlib.import_module("fifty_agent_sdk.audit.sql")
...
def test_lazy_attribute_access_on_audit_package_triggers_extras_hint(
    ...
    """``fifty_agent_sdk.audit.SqlAuditSink`` (lazy) raises the same ImportError."""
    # `fifty_agent_sdk.audit` itself does not import sqlalchemy, so a fresh import
    # of it must succeed even while sqlalchemy is blocked.
    sys.modules.pop("fifty_agent_sdk.audit", None)
    audit_pkg = importlib.import_module("fifty_agent_sdk.audit")
...
def test_top_level_lazy_attribute_access_triggers_extras_hint(
    ...
    """``fifty_agent_sdk.SqlAuditSink`` (top-level lazy) raises the same ImportError."""
...
def test_lazy_attribute_access_for_audit_metadata_triggers_extras_hint(
    ...
    """``fifty_agent_sdk.audit_metadata`` lazy access also surfaces the extras hint."""
    sys.modules.pop("fifty_agent_sdk", None)
    pkg = importlib.import_module("fifty_agent_sdk")
...
def test_audit_package_import_still_works_without_extras(
    ...
    """``import fifty_agent_sdk.audit`` succeeds and exposes the dependency-free surface."""
    sys.modules.pop("fifty_agent_sdk.audit", None)
    audit_pkg = importlib.import_module("fifty_agent_sdk.audit")
